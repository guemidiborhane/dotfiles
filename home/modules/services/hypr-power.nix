{ pkgs, ... }:
let
  hyprPower = pkgs.writeShellScriptBin "hypr-power" ''
    set -euo pipefail
    ...
    if [ -z "$BATTERY" ]; then
      echo "[hypr-power] No battery found, exiting..."
      exit 0
    fi
...
in
{
  ...
  systemd.user.services.hypr-power = {
    Unit = {
    ...
    Service = {
      Type = "simple";
      ExecStart = "${hyprPower}/bin/hypr-power";
      Restart = "on-failure";
      RestartSec = "3s";
    };
