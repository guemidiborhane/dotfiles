...
if [ -z "${1:-}" ]; then
    HOSTS=$(tomlq -r '. | keys[]' "$CONFIG_FILE")
    if [ -z "$HOSTS" ]; then
        gum style --foreground 196 "No hosts found in $CONFIG_FILE"
        exit 1
    fi
...
if tomlq -e ".\"$NAME\"" "$CONFIG_FILE" >/dev/null 2>&1; then
    ...
    RESOLVED=$(tomlq -r ". | to_entries[] | select(.value.hostname == \"$NAME\") | .key" "$CONFIG_FILE" | head -n1)
    if [ -z "$RESOLVED" ]; then
...
# Remove directory
if [ -d "modules/system/hardware/hosts/$RESOLVED" ]; then
    gum spin --spinner dot --title "Removing host directory..." -- rm -rf "modules/system/hardware/hosts/$RESOLVED"
fi
